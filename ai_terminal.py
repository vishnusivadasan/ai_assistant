...
class MemoryManager:
    ...
    def extract_entities_with_ai(self, command: str, result: str) -> List[Dict]:
        ...
        """
        if not hasattr(self, 'ai_terminal') or not self.ai_terminal:
            return []
            
        self.ai_terminal.log_debug(f"Extracting entities using API for command: {command}")
        ...
        dir_context = self.ai_terminal.get_directory_context()
...
Extract all entities (files, directories, etc.) being operated on in this command."""
        ...
        try:
            ...
            response = self.ai_terminal.call_api_with_animation(
                openai.chat.completions.create,
                model=openai_model,
                messages=messages,
                response_format={"type": "json_object"},
            ...
            self.ai_terminal.log_debug(f"API extracted {len(entities)} entities from command")
            return entities
            
        except Exception as e:
            error_msg = f"Error extracting entities from command: {str(e)}"
            if hasattr(self, 'ai_terminal') and self.ai_terminal:
                self.ai_terminal.log_debug(error_msg)
            return []
